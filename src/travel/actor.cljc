(ns travel.actor
  "TravelConsultantsActor — the ISCO-08 4221 community travel actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one travel operation request
  ...
  The unconditional invariant: the TravelConsultantsAdvisor can never
  directly commit a record the TravelConsultantsGovernor refuses —
...
(defn build-graph
  "Build a compiled TravelConsultantsActor graph. `store` implements
  `travel.store/Store`. `advisor` implements
  `travel.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :inventory-id (:inventory-id proposal)
                                    :payload proposal}]
